...
DEFAULT_CONFIG = "configs/experiments/single_month.yaml"
EXPERIMENT = "single_month_cross_region"
CITY_ALIASES = {
    "chicago": "chicago",
    "nyc": "nyc",
    "new_york": "nyc",
...
def _task_city(task: ExperimentTask) -> str:
    parts = task.fold.split("_", 1)
    if len(parts) != 2:
        raise ValueError(f"Unexpected single-month fold: {task.fold}")
    return parts[1]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run single-month within-city cross-region experiments")
