...
DEFAULT_CONFIG = "configs/experiments/single_month_multiseed.yaml"
EXPERIMENT = "single_month_cross_region_multiseed"
CITY_ALIASES = {
    "chicago": "chicago",
    "nyc": "nyc",
    "new_york": "nyc",
...
def _task_city(task: ExperimentTask) -> str:
    ...
    if not separator or "_" not in prefix:
        raise ValueError(f"Unexpected multi-seed fold: {task.fold}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run multi-seed single-month cross-region experiments")
