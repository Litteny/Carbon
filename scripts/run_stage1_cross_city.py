...
DEFAULT_CONFIG = "configs/experiments/stage1_cross_city.yaml"
EXPERIMENT = "cross_city"
CITY_ALIASES = {
    "chicago": "chicago",
    "nyc": "nyc",
    "new_york": "nyc",
...
def _task_city(task: ExperimentTask) -> str:
    prefix = "target_"
    if not task.fold.startswith(prefix):
        raise ValueError(f"Unexpected cross-city fold: {task.fold}")
    return task.fold[len(prefix):]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Stage 1 leave-one-city-out cross-city experiments")
