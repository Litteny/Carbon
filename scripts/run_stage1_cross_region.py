...
DEFAULT_CONFIG = "configs/experiments/stage1_cross_region.yaml"
EXPERIMENT = "cross_region"
CITY_ALIASES = {
    "chicago": "chicago",
    "nyc": "nyc",
    "new_york": "nyc",
...
def _task_city(task: ExperimentTask) -> str:
    if "_" not in task.fold:
        raise ValueError(f"Unexpected cross-region fold: {task.fold}")
    return task.fold.split("_", 1)[0]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Stage 1 within-city cross-region pilot experiments")
