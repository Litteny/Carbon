...
from carbon_transfer.experiments import ExperimentTask, aggregate_for_config, discover_tasks, run_experiment
from carbon_transfer.models.registry import list_model_ids


DEFAULT_CONFIG = "configs/experiments/three_year_cross_region_2021_2023.yaml"
EXPERIMENT = "three_year_cross_region_2021_2023"
CITY_ALIASES = {
    "chicago": "chicago", "nyc": "nyc", "new_york": "nyc",
...
def _task_city(task: ExperimentTask) -> str:
    ...
    if not separator or "_" not in prefix:
        raise ValueError(f"Unexpected three-year fold: {task.fold}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run 2021-2023 within-city cross-region experiments")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for argument, key in (
        (args.epochs, "max_epochs"), (args.patience, "patience"),
    ...
    protocol_tasks = discover_tasks(
        config, models=args.models, seeds=args.seeds, experiments=[EXPERIMENT],
    )
    ...
    selected_tasks = [
        task for task in protocol_tasks if cities is None or _task_city(task) in cities
    ]
    ...
    return run_experiment(
        config, selected_tasks, force=args.force, aggregate_tasks=protocol_tasks,
    )
