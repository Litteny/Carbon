...
DEFAULT_CONFIG = "configs/experiments/stage1_cross_city_precomputed.yaml"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build fold-specific frozen POI embeddings for an experiment protocol",
    )
    ...
    parser.add_argument(
        "--work-dir",
        default="/tmp/carbon_poi_embeddings",
        help="Local temporary directory for embedding files (use local scratch, not Ceph)",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    split_root = project_path(config["split_dir"]) / str(config["split_experiment"])
